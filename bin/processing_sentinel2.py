...
def insertSargazoDB(conect,cur,crs,pathInput):
    ...
    with open(pathInput, 'r') as f:
        ...
        for row in reader:
            ...
            cur.execute("INSERT INTO sargazo (idpoligono, tile, fecha, fechadia, areakm2, distcosta_km, lugar, nom_playa, geom) VALUES ('"+row[0]+"', '"+row[1]+"', '"+row[2]+"', '"+row[3]+"', '"+row[4]+"', '"+row[5]+"', '"+row[6]+"', '"+row[7]+"', ST_Transform(ST_GeomFromText('"+row[8]+"',"+crs+"),4326))")
