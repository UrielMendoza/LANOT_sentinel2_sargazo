...
def mascarasVectoriales(tile,anio,fecha,fechaProc,bufferLM,pathLM,pathTmp,pathOutput,pathOutputEmpty,pathOutputPeta):
    ...
    # SIN DETFOO
    df = gpd.read_file(pathTmp+'alg_mask_filter_tmp_sar.json')
    df_mask = gpd.read_file(pathLM+'land_UTM16N_20m_SPlaya_2021.geojson')
    res_difference = gpd.overlay(df, df_mask, how='difference')
    print('=============================================')
...
#                    contEnt += 1   
                # SCL
                elif (scl[i,j] == 8) or (scl[i,j] == 9) or (scl[i,j] == 10) or (scl[i,j] == 11):
                    nuMask[i,j] = 0
...
def insertSargazoDB(conect,cur,crs,pathInput):
    ...
    with open(pathInput, 'r') as f:
        ...
        for row in reader:
            ...
            cur.execute("INSERT INTO sargazo (idpoligono, tile, fecha, fechadia, area_km2, distcosta_km, lugar, nom_playa, geom) VALUES ('"+row[0]+"', '"+row[1]+"', '"+row[2]+"', '"+row[3]+"', '"+row[4]+"', '"+row[5]+"', '"+row[6]+"', '"+row[7]+"', ST_Transform(ST_GeomFromText('"+row[8]+"',"+crs+"),4326))")
